...
bool BTGAP::init() {
    if (self == 0) {
        ESP_LOGI(BT_GAP_TAG, "Initializing GAP");
        self = this;
        done = false;
        err = 0;
        peers.clear();
...
bool BTGAP::startInquiry() {
    ESP_LOGI(BT_GAP_TAG, "Starting inquiry");
    done = false;
...
void BTGAP::btGapCallback(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param) {
    ...
    case ESP_BT_GAP_DISC_RES_EVT:
        ...
        for (int i = 0; i < param->disc_res.num_prop; i++) {
            BTPeerInfo peer;
            // ESP_LOGI(BT_GAP_TAG, "type=%d", param->disc_res.prop[i].type);
            if (param->disc_res.prop[i].type == ESP_BT_GAP_DEV_PROP_BDNAME) {           
                ...
                ESP_LOGI(BT_GAP_TAG, "name='%s'", peer.name.c_str());
            }
    ...
    case ESP_BT_GAP_DISC_STATE_CHANGED_EVT:
        ESP_LOGI(BT_GAP_TAG, "ESP_BT_GAP_DISC_STATE_CHANGED_EVT state:%d", param->disc_st_chg.state);
    ...
    case ESP_BT_GAP_PIN_REQ_EVT:{
        ESP_LOGI(BT_GAP_TAG, "ESP_BT_GAP_PIN_REQ_EVT min_16_digit:%d", param->pin_req.min_16_digit);
    ...
    case ESP_BT_GAP_MODE_CHG_EVT:
        ESP_LOGI(BT_GAP_TAG, "ESP_BT_GAP_MODE_CHG_EVT mode:%d", param->mode_chg.mode);
        break;

    default:
        ESP_LOGI(BT_GAP_TAG, "Unknown event: %d", event);
        break;
    }
}
